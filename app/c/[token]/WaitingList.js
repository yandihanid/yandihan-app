...
export default function WaitingList({ storeId, token }) {
  ...
  useEffect(() => {
    ...
    load()
    const channel = supabase.channel('waiting-' + storeId).on('postgres_changes', { event: '*', schema: 'public', table: 'transactions', filter: `store_id=eq.${storeId}` }, () => load()).subscribe()
    ...
    <div className="card" style={{ maxWidth: '500px', margin: '1rem auto' }}>
      ...
      {tickets.map(t => (
        <div key={t.id} style={{ border: '1px solid #eee', borderRadius: '8px', padding: '0.75rem', marginBottom: '0.5rem' }}>
          ...
          <button onClick={() => markDone(t.id)} style={{ marginTop: '0.5rem', padding: '0.25rem 0.75rem', background: '#22c55e', color: 'white', border: 'none', borderRadius: '6px' }}>Selesai / Panggil</button>
        </div>
      ))}
    </div>
  )
