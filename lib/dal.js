...
const STORE_COLUMNS =
  'id, name, address, phone, user_id, unique_code, subscription_tier, subscription_end_date, receipt_required, require_sub_product, require_customer_name, waiting_list_enabled, pelanggan_enabled, visit_threshold, discount_percent, created_at'
...
export async function requireProductOwnership(productId) {
  ...
  const { data: product } = await supabase
    .from('products')
    .select('id, store_id, name, price, stock, stores!inner(user_id)')
    .eq('id', productId)
